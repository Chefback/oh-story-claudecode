...
> 适用范围：仅长篇导入。短篇不产 `追踪/角色状态.md`（依据 `../../story-long-write/references/state-tracking.md`：「本节仅适用于长篇写作。短篇通常不需要独立的角色状态追踪文件」）。
>
...
复用 `../../story-long-write/references/state-tracking.md` 规则：**只追踪出场 ≥3 次或有独立剧情线的角色**。
...
**超过 10 条时按以下规则压缩**（复用 `../../story-long-write/references/state-tracking.md` 规则）：
...
严格对齐 `../../story-long-write/references/artifact-protocols.md:237-252` 的 `角色状态.md` 标准模板，逐字段一一对应，不新增、不删减字段。
...
- [ ] 所有字段与 `../../story-long-write/references/artifact-protocols.md:237-252` 模板逐字段对应，无字段缺失
- [ ] 不确定字段已加 `[导入反推]` 标记
