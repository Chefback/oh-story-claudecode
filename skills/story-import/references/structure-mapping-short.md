...
| 拆书产物 | 短篇工程文件 | 转换方式 |
|---------|------------|---------|
| 原文全文（`拆文库/{书名}/原文/`） | `{标题}/正文.md` | 单文件迁移，按 `../../story-short-write/references/format-and-structure.md` 规范化格式（小节标记 `###1.`、段间无空行、半角双引号对话）；原文已是成稿，**不重写内容，只规范格式** |
...
将原文迁移为单文件 `正文.md`，格式按 `../../story-short-write/references/format-and-structure.md` 规范化：

| 规范项 | 要求 |
|--------|------|
| 小节标记 | `###1.` `###2.` `###3.`（黑岩/点众主流格式；用户指定平台时按 `../../story-short-write/references/format-and-structure.md` 平台覆盖表切换） |
| 段落长度 | 每段 ≤ 60 字，超出必须拆分 |
...
- [ ] `正文.md` 单文件存在且格式符合 `../../story-short-write/references/format-and-structure.md`（小节标记、段长、段间无空行、引号格式）
