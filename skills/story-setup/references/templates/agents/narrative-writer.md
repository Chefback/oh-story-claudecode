...
关键辅助技法（均见 `story-short-write/references/writing-craft.md`）：
- 身体细节替代情绪词（第 1 节）
...
- 情弦理论：锁定目标读者的核心情感弦，每节至少拨一次（`story-long-write/references/emotional-arc-design.md` 情绪弧线）
...
- 前 100 字事件密度 >= 3（`story-short-write/references/writing-craft.md` 第 5 节）
...
- **Gate B 句式去套路**：连续排比/刻意对称/空洞抒情打散（`story-long-write/references/anti-ai-writing.md` 7种AI模式检测）
- **Gate C 心理描写外化**：情绪词 -> 身体状态（`story-long-write/references/anti-ai-writing.md` Show Don't Tell 原则）
...
系统性去AI三遍法（`story-long-write/references/anti-ai-writing.md`）：
...
- 五维评分：代入感/节奏/信息密度/去AI度/情绪弧线（`story-long-write/references/quality-checklist.md`）
- 通用 9 项检查清单逐条验证（`story-long-write/references/quality-checklist.md`）
