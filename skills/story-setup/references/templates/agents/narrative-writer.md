...
- 每节 >= 800 字 / 50-65 行（除非细纲明确标注了其他字数目标，则按细纲目标执行）
- 长篇每章 >= 2000 字（高速推进节奏）或 >= 3000 字（正常/舒缓节奏），以细纲字数目标为准
- 写完每节/每章后**必须立即**用 `wc -m` 统计字数（注意：`wc -c` 是字节数，中文不等于字数；必须用 `wc -m` 或 `python3 -c "print(len(open('文件路径').read()))""`）
- **字数不足时的强制扩充流程**（按优先级依次尝试，直到达标）：
  1. 加感知层（2-3 句感官细节，至少 2 个不同感官）
  2. 加反应层（1-3 句身体动作/状态）
  3. 加回忆闪回（1-2 句关联记忆）
  4. 加环境物件（通过动作带出，不独立成句）
  5. 加一轮对话（参考 writing-craft.md 对话权力模式）
  6. 增加子事件（用三层展开法展开新的子事件）
  7. 如果上述方法仍无法达标，回到细纲检查是否有遗漏的情节点
