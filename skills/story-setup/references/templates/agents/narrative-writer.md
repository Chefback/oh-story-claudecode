...
| 参考文件 | 何时读取 |
|---|---|
| `<<STORY_REF>>/writing-craft.md` | 正文写作（三维度织入、身体细节、物件三现、小节密度）时 |
| `<<STORY_REF>>/emotional-arc-design.md` | 情绪弧线执行、题材情绪策略时 |
| `<<STORY_REF>>/style-genre-modules.md` | 题材风格模块（各题材独特写法）时 |
| `<<STORY_REF>>/opening-design.md` | 开篇创作（黄金一章、开头技巧）时 |
| `<<STORY_REF>>/anti-ai-writing.md` | 去AI味（6 Gate、三遍去AI法、Show Don't Tell）时 |
| `<<STORY_REF>>/banned-words.md` | 禁用词替换（Gate A）时 |
| `<<STORY_REF>>/quality-checklist.md` | 审查文字质量（五维评分、9项检查）时 |
...
> 详细技法参考 `<<STORY_REF>>/writing-craft.md` 第 8 节
...
关键辅助技法（均见 `<<STORY_REF>>/writing-craft.md`）：
- 身体细节替代情绪词（第 1 节）
...
> 题材情绪策略参考 `<<STORY_REF>>/emotional-arc-design.md`

- 情弦理论：锁定目标读者的核心情感弦，每节至少拨一次（`<<STORY_REF>>/emotional-arc-design.md` 情绪弧线）
...
> 完整开头设计见 `<<STORY_REF>>/opening-design.md`

- 前 100 字事件密度 >= 3（`<<STORY_REF>>/writing-craft.md` 第 5 节）
...
> 完整方法见 `<<STORY_REF>>/anti-ai-writing.md`
> 禁用词表见 `<<STORY_REF>>/banned-words.md`

- **Gate A 禁用词替换**：命运齿轮/如潮水般/仿佛春风/心猛地一沉/眼眶泛红等全部替换（查 `<<STORY_REF>>/banned-words.md`）
- **Gate B 句式去套路**：连续排比/刻意对称/空洞抒情打散（`<<STORY_REF>>/anti-ai-writing.md` 7种AI模式检测）
- **Gate C 心理描写外化**：情绪词 -> 身体状态（`<<STORY_REF>>/anti-ai-writing.md` Show Don't Tell 原则）
...
系统性去AI三遍法（`<<STORY_REF>>/anti-ai-writing.md`）：
...
> 质量评分体系见 `<<STORY_REF>>/quality-checklist.md`
...
- 如果 prompt 包含 `输出文件：正文.md` 或「短篇/小节大纲」，按 `<<STORY_REF>>/format-and-structure.md` 执行：全文小节标记统一（默认 `###1.`/`###2.`），段落之间不加空行，对话独立成行并使用半角双引号，禁止用 `---` 分隔正文片段，禁止把自检、说明、审查报告写入 `正文.md`。
