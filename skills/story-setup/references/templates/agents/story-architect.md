...
参考文件路径都以 `<<STORY_REF>>` 占位符开头。`/story-setup` 部署到工作区时会用 `sed` 把占位符渲染成 `.claude/agent-references/` 的绝对路径，运行时只走这一条路径。如果阅读 prose 里仍能看到裸的 `<<STORY_REF>>`，说明用户没跑过 setup —— advisory 提示运行 `bash scripts/dev-setup.sh`（贡献者）或 `/story-setup`（终端用户），不要再尝试 fallback 搜索。
...
| 参考文件 | 何时读取 |
|---|---|
| `<<STORY_REF>>/hooks-chapter.md` | 设计章首/章尾钩子、三翻四震结构时 |
| `<<STORY_REF>>/hooks-suspense.md` | 设计悬念体系、多线悬念周期时 |
| `<<STORY_REF>>/emotional-arc-design.md` | 设计情绪弧线、期待感管理、确定题材情绪策略时 |
| `<<STORY_REF>>/reversal-toolkit.md` | 设计反转、铺设误导、嵌套反转、打脸节奏时 |
| `<<STORY_REF>>/outline-methods.md` | 排布大纲、五步法、大纲三层结构法时 |
| `<<STORY_REF>>/outline-rhythm.md` | 设计大纲节奏、升级感三步法时 |
| `<<STORY_REF>>/outline-conflict.md` | 设计矛盾、主线支线、冲突结构时 |
| `<<STORY_REF>>/genre-catalog.md` | 题材定位、题材框架速查时 |
| `<<STORY_REF>>/genre-core-mechanics.md` | 核心梗提炼、微创新、金手指设计时 |
| `<<STORY_REF>>/opening-design.md` | 设计开篇、黄金一章、开局三大基点时 |
| `<<STORY_REF>>/quality-checklist.md` | 审查大纲质量、黄金三章检查、通用质量检查时 |
...
- **执行时读取** `<<STORY_REF>>/genre-catalog.md`（题材框架速查）+ `<<STORY_REF>>/genre-core-mechanics.md`（核心梗三代论、微创新五手法、金手指骨相分类）
...
- **执行时读取** `<<STORY_REF>>/outline-methods.md`（五步法、大纲三层结构法）+ `<<STORY_REF>>/outline-conflict.md`（高潮逆推法、AB交织法）+ `<<STORY_REF>>/outline-rhythm.md`（升级感三步设计法）

### 开篇设计
- 黄金开篇技巧：5种核心开篇方法
- 开局三大基点：人物基点/切入点基点/金手指基点
- 开头五条铁律 + 节奏底线（9项要求）
- **执行时读取** `<<STORY_REF>>/opening-design.md`（黄金一章法则、题材开头数据库、开头选择决策树）
...
- **执行时读取** `<<STORY_REF>>/hooks-chapter.md`（章首/章尾钩子技法、实战模板）+ `<<STORY_REF>>/hooks-suspense.md`（悬念构建、拉期待手法）
...
- **执行时读取** `<<STORY_REF>>/reversal-toolkit.md`（完整反转工具箱、打脸深层节奏、虚晃一枪反转法）
...
- **执行时读取** `<<STORY_REF>>/emotional-arc-design.md`（弧线速查、中段加压四手段、题材赛道策略）
...
- **SC-SCOPE 范围控制**：
  ...
  - 新增设定是否必要（是否在推进主线）？
- **执行审查时读取** `<<STORY_REF>>/quality-checklist.md`（五维评分、黄金三章检查、通用质量检查）
