...
读取参考文件时，优先从项目根目录下的 `.claude/skills/` 或 `skills/` 解析下方规范路径；不要只读取裸文件名。若当前工具只接受相对路径，先尝试 `.claude/skills/{规范路径}`，再尝试 `skills/{规范路径}`，最后用 Glob/Grep 搜索 `*/{规范路径}`。
...
- **执行时读取** `story-long-write/references/genre-catalog.md`（题材框架速查）+ `story-long-write/references/genre-core-mechanics.md`（核心梗三代论、微创新五手法、金手指骨相分类）
...
- **执行时读取** `story-long-write/references/outline-methods.md`（五步法、大纲三层结构法）+ `story-long-write/references/outline-conflict.md`（高潮逆推法、AB交织法）+ `story-long-write/references/outline-rhythm.md`（升级感三步设计法）

### 开篇设计
- 黄金开篇技巧：5种核心开篇方法
- 开局三大基点：人物基点/切入点基点/金手指基点
- 开头五条铁律 + 节奏底线（9项要求）
- **执行时读取** `story-long-write/references/opening-design.md`（黄金一章法则、题材开头数据库、开头选择决策树）
...
- **执行时读取** `story-long-write/references/hooks-chapter.md`（章首/章尾钩子技法、实战模板）+ `story-long-write/references/hooks-suspense.md`（悬念构建、拉期待手法）
...
- **执行时读取** `story-long-write/references/reversal-toolkit.md`（完整反转工具箱、打脸深层节奏、虚晃一枪反转法）
...
- **执行时读取** `story-long-write/references/emotional-arc-design.md`（弧线速查、中段加压四手段、题材赛道策略）
...
- **SC-SCOPE 范围控制**：
  ...
  - 新增设定是否必要（是否在推进主线）？
- **执行审查时读取** `story-long-write/references/quality-checklist.md`（五维评分、黄金三章检查、通用质量检查）
