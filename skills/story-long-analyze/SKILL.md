...
| 7 | 语料库索引更新 | 概要.md + 拆文报告.md + 文风.md | 更新 `拆文库/语料库索引.md`：追加本书元数据条目（标号/题材/核心梗/可借鉴点/文风特点），更新按题材/情绪/技法的索引段落 | 索引条目追加完成 |
...
Stage 6 完成后（或 Stage 6 失败后），自动更新项目根目录的 `拆文库/语料库索引.md`：

1. 读取 `拆文库/{书名}/概要.md`（长篇）或 `拆文库/{书名}/拆文报告.md`（短篇）提取元数据
2. 从 `拆文库/{书名}/拆文报告.md` 提取核心梗、可借鉴套路、写法技巧
3. 从 `拆文库/{书名}/文风.md`（如存在）提取文风特点
4. 按题材分配标号（`{题材前缀}{序号}`），追加到 `拆文库/语料库索引.md` 的索引总览表和详细条目区
5. 更新按题材/情绪/技法的索引段落
6. 索引不存在时自动创建（模板见各分析 skill 的 demo 输出）

**语料库索引不存在时**：如果 `拆文库/语料库索引.md` 不存在，自动创建并填入当前书目的条目。已有时追加新条目，不覆盖已有条目。
