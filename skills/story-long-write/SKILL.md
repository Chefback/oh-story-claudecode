...
**注意事项**：前 30 章（约 12 万字）不要改大纲。写够 30 章自然会发现问题和大方向。
...
5. **字数验证**（写作完成后的第一件事）：用 `wc -m`（或 `python3 -c "print(len(open('正文文件路径').read()))""`）统计本章实际字数。如果字数 < 细纲目标的 90%，必须强制扩充（加感知层/反应层/回忆闪回/环境物件/对话/子事件），直到达标后再进入步骤 6。
6. **检查**：章尾是否有钩子、爽点是否到位
