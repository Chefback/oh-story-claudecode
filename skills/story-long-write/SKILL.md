...
| **日更续写** | 关键词（"日更"/"续写"/"继续写"/"继续"）**且**项目已有正文+追踪 | 加载 `references/workflow-daily.md`（⚠️ 必须从 Step 0/1 开始完整执行，不得跳过上下文加载直接写作） |
...
每完成 20 章或一个卷结束时，对 `追踪/上下文.md` 做一次轻量归档：保留最近 5 章详记，将更早内容压缩到 `追踪/归档/第XXX-YYY章.md`，并在上下文中保留归档索引。伏笔、时间线、角色状态仍以当前文件为准，不把活跃线索移入归档。**日更模式下**，workflow-daily Step 0 会在每次日更开始时自动检查并执行压缩，无需手动触发。
