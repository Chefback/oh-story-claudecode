...
> 产线映射：拆文产出→设定集(Setting Collection) — 复用拆文库的结构化资产
...
⚠️ **大纲三检（每卷/每章设计前必答）**：① 本卷核心冲突是什么？② 卷节奏（起承转合）哪段加速哪段减速？③ 本卷需要新埋设的伏笔有哪些？上一卷待回收的伏笔如何处理？
...
**缺失文件回退**：所有新增文件是可选增强。缺失时 agent 降级到当前行为，不报错不阻塞——情绪/反转信息在卷纲或大纲中体现，伏笔/时间线/角色状态不检查，对标结构化子目录跳过（回退到拆文报告.md）。
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（检查 `.claude/agents/story-explorer.md` 是否存在），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (9) `对标/{对标书名}/剧情/故事线.md`（如不存在，查找 `拆文库/{对标书名}/剧情/故事线.md`）— 剧情线索引，用于确定本章涉及哪些剧情线
   - (10) `对标/{对标书名}/剧情/{相关剧情线}.md`（如不存在，查找 `拆文库/{对标书名}/剧情/{相关剧情线}.md`）— 从索引中选择与本章相关的剧情线文件
   - (11) `对标/{对标书名}/设定/金手指.md` 或 `世界观.md`（如不存在，查找 `拆文库/{对标书名}/设定/` 下同名文件）— 从拆文产出的设定中获取参考
3. **准备层**（写作前的系统化准备，确保 LLM 在动笔前具备完整创作意图）：
   ...
   - 3.3 **指令确认**：综合细纲+最简记忆包+模块召回结果，确认本章节奏（快/慢）和情绪目标，用一句话概括本章写作意图
...
> 产线映射：AI质检(AI Guardian) — 时空一致性 + 人物逻辑 + 大纲贴合度
