...
> **日更模式准备步骤**：每章写作前执行 3 步（状态筛选 + 对标写作手法召回 + 意图确认），嵌入 Step 2 逐章执行循环内。日更模式**保留**对标结构化子目录检索（SKILL.md 步骤 3.2 的第二部分）：写作时仍需按「对标书路径查找」规则从 `对标/` 或 `拆文库/` 的结构化子目录中检索与本章相关的模块（角色/剧情/设定/章节），重点关注对标书的**写作手法**（叙事节奏、镜头切换、对话风格、情绪铺陈方式），输出"对标写作手法召回摘要"（最多5条，标明路径+可借鉴的写作手法），作为正文写作的风格参考。情绪-技法匹配（步骤 3.2 的第一部分）通过 Step 2.2 的意图确认覆盖。完整准备层逻辑见 `SKILL.md` 的 Phase 4 正文写作辅助部分。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **2.2.1 对标写作手法召回**：按「对标书路径查找」规则从 `对标/` 或 `拆文库/` 的结构化子目录中检索与本章最相关的模块，**重点提取对标书的写作手法**（叙事节奏、镜头切换方式、对话风格、情绪铺陈技巧、白描手法、场景转换方式等），输出"对标写作手法召回摘要"（最多5条，标明路径+可借鉴的写作手法），作为 Step 2.3 意图确认和正文写作的风格参考。如果对标书路径下存在原文，优先读取与本章位置对应的章节原文，分析其具体写法（开头方式、段间衔接、情绪节奏、对话与叙述比例）
   - **2.3 意图确认**（一句话）：从细纲「目标情绪」字段确认本章情绪目标，综合状态筛选结果和对标写作手法召回结果，用一句话确认本章写作意图（情绪+节奏+借鉴的写作手法）。例：「快节奏打脸——借鉴对标书第15章的压迫式短句+反转节奏，读者等了三章，这章必须一拳到位」
