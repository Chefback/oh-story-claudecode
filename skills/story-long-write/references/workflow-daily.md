...
2. **逐章执行**（以下每步在每章循环内执行）：
   - 读细纲 → 按需加载角色设定
   - **章节名去重检查**：确认本章章名与 `大纲/` 目录中已有细纲的章名不重复。如重复，按 outline-methods.md「章节名策略」中的去重规则重新命名
   - **2.1 状态筛选**：从已加载的上下文中筛选。如果已加载的上下文足够，直接使用，不额外加载文件。角色最新状态优先从 `追踪/角色状态.md` 筛选（如不存在则从角色设定推断），待回收伏笔从 `追踪/伏笔.md` 筛选
   - **2.2 意图确认**（一句话）：从细纲「目标情绪」字段确认本章情绪目标，综合状态筛选结果，用一句话确认本章写作意图（情绪+节奏）
