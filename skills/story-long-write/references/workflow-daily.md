...
> **日更模式准备步骤**：每章写作前执行 2 步（状态筛选 + 意图确认），嵌入 Step 2 逐章执行循环内。完整 3 步准备层见 SKILL.md 单章写作步骤 3。如果 SKILL.md 的准备层与本文档同时出现在上下文中，以本文档为准。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   - 读细纲 → 按需加载角色设定
   - **2.1 状态筛选**：从已加载的上下文（上下文.md + 伏笔.md）中提取本章涉及角色的最新状态、待回收伏笔。如果已加载的上下文足够，直接使用，不额外加载文件。如果角色状态.md 中有本章涉及角色的更新，按需加载该文件筛选
   - **2.2 意图确认**（一句话）：综合细纲+状态筛选结果，用一句话确认本章写作意图（情绪+节奏）
   - 写正文 → **字数验证（`wc -m`统计，< 目标90%则强制扩充）** → 检查钩子/爽点 → 禁用词扫描
   - 每章写完后**立即更新**以下文件：
     - `追踪/伏笔.md`（新增/回收伏笔）
     - `追踪/时间线.md`（记录事件时序）
