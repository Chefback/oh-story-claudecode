...
6. **禁止与已有章节标题重复或高度相似**：新建细纲前，必须扫描 `大纲/` 目录中已有的所有细纲文件名和章名，确保新标题与已有标题不重复
7. **禁止使用泛化标题**：不允许使用"意外""发现""危机""转折""阴谋""秘密"等可套用在任何章节的泛化词作为主标题。必须加入具体的、只属于本章的元素（人名、地名、物件、具体事件）
8. **标题必须反映本章独特内容**：读完标题应能唯一对应到这一章，而非其他任何章节。检验方法：如果换到其他章节这个标题也成立，就不够具体
9. **同卷内标题风格差异化**：同一卷内，相邻 5 章的标题在句式结构、用词、情绪调性上应有明显区分。避免连续出现相同句式（如连续使用"XXX的YYY"结构）
10. **标题长度差异化**：同卷内标题字数应在 2-10 字间变化，避免所有标题长度雷同（如全部 4 字或全部 6 字）

### 章节名生成流程

每章命名时执行以下步骤：

1. 回顾已有章节标题列表（从 `大纲/细纲_第XXX章.md` 文件名和内容中提取）
2. 确认本章核心事件和爽点（从细纲内容提取）
3. 基于核心事件生成 3 个候选标题
4. 排除与已有标题重复或相似的候选
5. 选择最抓眼球且唯一对应本章的标题
6. 确认标题符合去重规则 6-10
