...
10. [结构类型速查](#结构类型速查)
11. [质量标准](#质量标准)
12. [与 SKILL.md / output-templates.md 的关系](#与-skillmd--output-templatesmd-的关系)
...
| 读者视角优先 | 每一步从读者体验出发——读者看到了什么？感受到了什么？为什么在意？不分析"作者想写什么"，分析"读者实际体验了什么" |
| 可借鉴性导向 | 每个分析点都要回答"我能从中学到什么？怎么用在我的下一篇里？"纯欣赏型分析没有意义 |
...
| 结构 | 作者怎么布局的？开端/发展/高潮/结局各占多少？ | 布局决定了节奏，不知道布局就不知道节奏 |
| 人物 | 谁是主人公？谁是主动人物？谁是被动人物？ | 不知道谁在推故事，就不知道故事怎么动的 |
| 情感线 | 读者的情绪是怎么被一步步拉扯的？ | 这是短篇的核心价值，拆不出来等于白拆 |
| 写作手法 | 用了什么手法让读者产生这种体验？ | 手法是可学的，不拆手法就没法借鉴 |
| 共鸣+爆点 | 为什么这篇能火？触发了什么共鸣？ | 不回答这个问题，拆文就变成了文学鉴赏 |
| 综合评估 | 我能学到什么？下一篇怎么用？ | 不能指导写作的拆文没有意义 |

### 拆解心态

1. **别当评论家，当学习者** — 不评价好坏，只拆解机制
2. **别从概念出发，从体验出发** — 先回忆自己读的时候哪里最激动/最心酸，再去找作者怎么做到的
3. **别求全面，求深度** — 一篇拆透一个点，比面面俱到但都浅更有价值
4. **带着问题拆** — "如果是我写这个题材，我能怎么用？"这个念头要贯穿始终
...
>
> **主动报应 vs 被动报应**：报应设计分两种——①主动报应：主角设局让反派自爆（如Demo 5香水、Demo 7比赛设局）；②被动报应自循环：主角仅逃离/重建，报应来自施暴者内部反目（如白栀报复周燃）。被动报应更真实但爽感较低，分析时需标注类型。
...
**套娃反转的因果链**：套娃反转式的核心质量取决于因果链强度。每个反转必须解释或加深前一个反转——反转A解释了反转B的动机，反转B加深了反转C的冲击。如果反转之间没有因果关联，只是随机堆砌，套娃结构就会崩塌。分析时必须检验：删除任一层反转，其他反转是否仍然成立？如果是，说明因果链断裂。

**隐性反转（回看反转）**：部分反转不是通过新信息揭示，而是通过"已有信息被重新理解"实现。开头给出的一句话/一个场景，在读者获得新认知后含义完全改变。这种反转不需要铺垫新线索，而是利用读者自身的认知升级。分析时需标注：回看反转的原文位置、初读理解、回看理解、触发认知升级的事件。
...
| 弹幕/元叙事旁白 | 将"读者评论"变成故事内的角色——提供任务、喜剧评论、元叙事、剧情预告。既是叙事工具也是喜剧引擎。弹幕本身可以有一条独立的"人格弧线"（从自信到崩溃） |
| 反差萌循环 | "想A→做到B→对方反应C"的循环，核心是意图与结果的持续错位。角色越想装一种人，越暴露真实的自己。每次循环需升级 |

每项手法标注：使用位置、效果评价、可借鉴点。
...
**二次阅读设计**：部分故事的开头包含"回看反转"——开头的某句话在初读时是一种含义，读完真相后含义完全改变。这种设计创造了二次阅读价值（读者回去重看开头时获得全新体验）。分析时标注：哪些开头语句具有二次阅读价值？真相揭示后含义如何变化？
...
- 五维评分（开头吸引力、情感拉扯力、反转设计、节奏控制、结尾余韵）
  > **甜宠/喜剧类替代维度**：当故事类型为甜宠/喜剧/反差萌时，"反转设计"维度天然低分（此类故事通常无反转），建议替换为"反差萌浓度"，"情感拉扯力"替换为"甜度曲线"。替代维度：开头吸引力、甜度曲线、反差萌浓度、节奏控制、结尾余韵
- 爆点性 + 话题性
- 共鸣层次分析
- 可借鉴结构（至少 3 条，含适用场景）
- 同类型写作建议
...
- 信息差式：双方信息不对称，读者在中间
- 推拉式：靠近又推开，反复循环
- 称呼操控式：不同角色对同一人使用不同称呼，每次称呼都是身份认同/权力关系的博弈。分析时追踪称呼变化轨迹（如"阿虎"vs"林川"），标注称呼转变的触发事件
...
**意象系统**：当多个物件/意象服务于同一主题时，不应单独分析每个物件，而应识别它们是否构成"意象系统"——多个物件从不同角度映射同一个核心主题。如拳套（身份错位）+纹身（被迫标记）+储物间（精神囚禁）共同构成"身份囚禁"的意象系统。分析时标注：系统主题、各物件的映射角度、系统整体效果。
...
| 双峰 | 两个高潮 | 复杂短篇 | 层次丰富 |
| **心酸双峰** | 两个非爽感高潮（如两次告别/两次跳楼），中间穿插短暂治愈 | BE/告别 | 意难平递进——第二次比第一次更痛 |
| **甜度阶梯** | 全正值，情绪强度持续上升但不经历负值 | 甜宠/喜剧 | 持续甜蜜升级，无虐感 |
| **混合型** | 上述形态的组合 | 视组合而定 | 需标注主要形态和次要形态 |
...
| 分类 | 定义 | 示例 |
|------|------|------|
| 主人公 | 故事第一视角人物或核心焦点 | 第一人称"我"、沈暮月 |
| 主动人物 | 主动推动剧情发展的关键人物 | 做出关键决定的角色 |
| 被动人物 | 被剧情推着走、被动承受的人物 | 被迫接受命运的角色 |
| 功能人物 | 承担特定叙事功能的辅助人物 | 传信者、对照组 |
